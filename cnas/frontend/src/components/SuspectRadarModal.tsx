...
export const SuspectRadarModal: React.FC<SuspectRadarModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/85 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-4xl bg-slate-900 border border-sky-500/40 rounded-2xl shadow-2xl flex flex-col overflow-hidden text-slate-100 max-h-[92vh]">
          ...
          >
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Enter suspect name (e.g. Rahul Verma, Priya Sharma) or phone (e.g. 9811011223)..."
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-400 hover:to-blue-500 text-white text-xs font-bold shadow-md transition-all cursor-pointer flex items-center gap-2 shrink-0"
            >
              {loading ? (
                <>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {result && result.target && (
            <>
                    ...
                    <button
                      onClick={() => {
                        ...
                        const params = new URLSearchParams({
                          ...
                          name: result.target.name || 'Tracked Target',
              ...
              {/* Section: Live Telecom / GPS Location Telemetry Card */}
              {result.latest_location ? (
                <div className="p-5 rounded-2xl bg-slate-950 border border-emerald-500/40 space-y-3 shadow-glowGreen">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs font-mono">
                    ...
                    <div className="p-3 rounded-xl bg-slate-900 border border-slate-800 md:col-span-2">
                      <span className="text-[10px] text-slate-500 block">Location / Toll / Tower Description</span>
                  ...
                  {result.latest_location.case_title && (
                    <div className="flex items-center justify-between text-[11px] font-mono pt-1 text-slate-400 border-t border-slate-800/80">
                      <span>Linked Operation Dossier: <strong className="text-amber-300">{result.latest_location.case_title}</strong></span>
                      <span className="text-emerald-400 font-semibold">● ACTIVE TELEMETRY PING</span>
                    </div>
                  )}
                </div>
              ) : (
